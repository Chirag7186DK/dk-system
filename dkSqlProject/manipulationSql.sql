...
-- TRUNCATE TABLE  `DK_USERORDERCART`;
...
WHERE 1  AND ss.id IN ('1','2') AND spa.shoptstore_id IN ('1','2')  
AND ss.country_city_area_affiliationId IN ('1') AND ccr.id IN ('1')  
AND pt.id IN ('11') AND ppc.product_typeid IN ('11')  
AND spa.product_typeid IN ('11')  
AND sppc.shopstores_producttype_affiliationid = spa.id 
AND sppl.shopstores_producttype_affiliationid = spa.id  
ORDER BY pt.id, ppc.id, spa.product_typeid, sppc.producttype_categoryid,sppfd.online_sellprice ASC, 
sppfd.product_discount ASC
